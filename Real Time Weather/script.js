...
humidityVal=document.getElementById('humididtyVal'),
pressureVal=document.getElementById('pressureVal'),
visiblityVal=document.getElementById('visibilityVal'),
...
function getWeatherDetails(name, lat, lon, country, state)
{
    ...
    WEATHER_API_URL=`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&cnt={cnt}&appid=${api_key}`,
    ...
    fetch(WEATHER_API_URL).then(res =>res.json()).then(data =>{
        ...
        currentWeatherCard.innerHTML=`
            ...
            <div class="card-footer">
                ...
                <p><i class="fa-light fa-locatiob-dot"></i>${name}, ${country}</p>
...
function getCityCoordinates() 
{
    ...
    cityInput.value='';
    if(!cityName)return;
    let GETCODING_API_URL=`http://api.openweathermap.org/geo/1.0/direct?q=${cityName}&limit=1&appid=${api_key}`;
...
function getUserCoordinates()
{
    navigator.geolocation.getCurrentPosition(position =>{
        ...
        if(error.code===error.PERMISSION_DENIED){
            alert('Geolocation permission denied. Please reset location permissiom to grant acces again')
        }
